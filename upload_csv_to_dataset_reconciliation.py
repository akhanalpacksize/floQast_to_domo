...
from config.env import BASE_URL, FloQast_Reconciliations_v2
...
def update_schema(name, updated_schema_column, token):
    ...
    response = requests.put(f'{BASE_URL}/datasets/{FloQast_Reconciliations_v2}', headers=header, json=payload)

    if response.ok:
        logger.info(f"updated_schema '{name}' with ID {FloQast_Reconciliations_v2}")
    else:
...
def generate_update_schema():
    ...
    dataset_name = "FloQast_Reconciliations"
    try:
...
def upload_csv():
    ...
    csv_path = os.path.join(current_directory, output_dir, Reconciliations)
    ...
    if os.path.isfile(csv_path):
        ...
        try:
            ...
            response = requests.put(f'{BASE_URL}/datasets/{FloQast_Reconciliations_v2}/data', headers=header, data=csv_data)

            # Check the API status
            if response.status_code == 204:
                logger.info("Data imported successfully")
            else:
                error_message = f'Error {response.status_code} for file {Reconciliations}: {response.text}'
                logger.error(f'Error {response.status_code} for file {Reconciliations}: {response.text}')
                send_email_error(error_message)
        ...
        error_message = f"CSV file {Reconciliations} not found in {output_dir} folder."
        logger.info(f"CSV file {Reconciliations} not found in {output_dir} folder.")
        send_email_error(error_message)
