...
def generate_schema_columns(header):
    ...
    for elem in header:
        if elem in ["id",]:
            column_type = "long"
        elif elem in ["_id", "displayName", "type", "valid", "dataProviderType",
                      "credentialsType","userId",'createdBy','modifiedBy']:
            column_type = "string"
        elif elem == 'updatedAt'or elem == 'modifiedAt':
...
def update_schema(name, updated_schema_column, token):
    ...
    response = requests.put(f'{BASE_URL}/datasets/{FloQast_Checklist_v2}', headers=header, json=payload)
...
def generate_update_schema():
    ...
    dataset_name = "FloQast_Checklist"
    try:
...
def upload_csv():
    ...
    if os.path.isfile(csv_path):
        ...
        try:
            ...
            response = requests.put(f'{BASE_URL}/datasets/{FloQast_Checklist_v2}/data', headers=header, data=csv_data)
